...
| 4 | `database` | `apps/backend/src/db/**` or any file with SQL/`query`/`withTransaction` | Positional `$1` parameters only, dynamic `IN` via `= ANY($1::int[])`, singleton pool, N+1 prevention, multi-row inserts, transactions, snake→camel mapping, money tolerance, sargable WHEREs, additive migrations |
...
| 8 | `testing` | test files, `npm test`, testable code edits | Vitest (BE node, FE jsdom+RTL+MSW), ≥80% on BE services+utils, `*.test.ts(x)`, MSW per api module, mock the pg pool with column projection, regression-test before bug fix |
| 9 | `security` | auth/SQL/config/middleware/validators/login/token/bcrypt code | Zod on every input, server recomputes (no client trust), parameterized SQL only, bcrypt+HMAC token, helmet CSP, 50kb body, rate limits, `dangerouslySetInnerHTML` forbidden, no `eval`, no logging bodies/headers, never return `pin_hash` |
