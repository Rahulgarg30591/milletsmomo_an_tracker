...
import { getMinimumSaleValue as getMinSaleValue } from '../services/minimumSaleValueService.js';
...
export async function getMinimumSaleValue(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    ...
    const result = await getMinSaleValue(date);
    res.json(result);
  } catch (err: any) {
    if (err.name === 'ZodError') {
      res.status(400).json({ error: 'Invalid date format. Use YYYY-MM-DD.' });
