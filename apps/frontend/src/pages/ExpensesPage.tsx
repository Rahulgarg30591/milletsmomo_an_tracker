...
export default function ExpensesPage() {
  ...
  const { data: existingData, isLoading } = useQuery({
    ...
    queryFn: () => getDayExpenses(targetDate),
    enabled: !!targetDate,
  });
  ...
  const updateExpense = useCallback((localId: number, field: 'description' | 'amount', value: string) => {
  ...
  const createCopyText = useCallback((): string => {
    ...
    const lines = [`Expenses`, toDDMMYYYY(targetDate)];
    valid.forEach((e, i) => {
  ...
  return (
    <Box sx={{ minHeight: '100vh', backgroundColor: 'background.default', p: 1.5, pb: 8, pt: 1, overflowY: 'auto', '&::-webkit-scrollbar': { display: 'none' }, scrollbarWidth: 'none' }}>
        ...
        {expenses.length === 0 && !isLoading ? (
          ...
          >
            <Typography sx={{ color: 'text.secondary', fontSize: '0.85rem' }}>
              No expenses added yet. Use presets or add manually.
            </Typography>
          </Paper>
        ) : (
